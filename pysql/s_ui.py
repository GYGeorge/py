...
class MyWindow(QtWidgets.QMainWindow, QtWidgets.QTabWidget):
    ...
    def buttonTest(self):
        ...
        sqlcourse = "select sc.Grade from sc,c where sc.Sno = '%s' and c.Cno = sc.Cno and c.Cname = '%s'" % (self.sno, self.cname)                
    ...
    def buttonchange(self):
        if self.changePW.text() == self.confirmPW.text():
            ...
            sqlstr = "select * from s where Sno = '%s' and passwd = '%s'" % (self.sno, self.OriginalPW.text()) 
            self.cur.execute(sqlstr)
            if self.cur.fetchone():
                 sqlstr = " update s set passwd = '%s' where Sno = '%s'" % (pw, self.sno)
                 self.cur.execute(sqlstr)
                 self.conn.commit()
                 sqlcheck = "select * from s where Sno = '%s' and passwd = '%s'" % (self.sno, pw)
